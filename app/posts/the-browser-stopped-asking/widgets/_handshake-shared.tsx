...
/**
 * Shared handshake primitives for the §4 widgets. HandshakeSteps consumes
 * the multi-pane wide+narrow variants; KeyDerivation consumes only the
 * server-side compute pane plus AcceptSettle.
 */
...
/** RFC 6455 §1.3 — the literal example key/accept pair. Used by HandshakeSteps. */
export const SPEC_SAMPLE: Computed = {
...
export async function makeAccept(key: string): Promise<string> {
...
/* -------------------------------------------------------------------------- */
/*                          Steps-only canvas (wide)                          */
/* -------------------------------------------------------------------------- */

export function StepsCanvasWide({
  step,
  computed,
}: {
  step: number;
  ...
  return (
    ...
    >
      <PanelBox x={BROWSER_X} y={20} w={BOX_W} h={PIPE_Y - 40} label="browser" />
      <RequestPane
        x={BROWSER_X + 12}
        y={52}
        w={BOX_W - 24}
      ...
      <PanelBox x={SERVER_X} y={20} w={BOX_W} h={PIPE_Y - 40} label="server" />
      <ComputeBox
        x={SERVER_X + 12}
        y={52}
        w={BOX_W - 24}
      ...
      {/* The pipe (TCP socket) — muted base + terracotta WS overlay strokes
          itself in once step ≥ 2, conveying the HTTP→socket transition as
          a visible act instead of a discrete colour swap. */}
      <line
        x1={PIPE_X1}
        x2={PIPE_X2}
        y1={PIPE_Y}
      ...
      {/* Step 1: request envelope travels →. */}
      <motion.g
        initial={false}
        animate={{
          opacity: step === 0 ? 1 : 0,
...
/* -------------------------------------------------------------------------- */
/*                       Steps-only canvas (narrow)                           */
/* -------------------------------------------------------------------------- */

export function StepsCanvasNarrow({
  step,
  computed,
}: {
  step: number;
  ...
  return (
    <div className="flex flex-col gap-[var(--spacing-sm)]">
      ...
      >
        ...
        {step >= 2 ? (
          <NarrowFrameTraffic pipeX={PIPE_X} y1={PIPE_Y1 + 8} y2={PIPE_Y2 - 8} />
        ) : null}
      ...
      {[
        ...
        return (
          <text
            ...
            fontSize={10}
            fill={
              highlight && isUpgrade ? "var(--color-accent)" : "var(--color-text)"
            }
...
export function ComputeBox({
  ...
  accept: string;
  active: boolean;
  /** When true, render the key/accept lines as `(generating)` placeholders. */
  placeholder?: boolean;
}) {
  return (
    <g transform={`translate(${x}, ${y})`}>
      <text
        ...
        fill="var(--color-text-muted)"
      >
        key = {placeholder ? "(generating)" : truncate(key96, 14)}
      </text>
      <text
        x={0}
        y={28}
...
function NarrowFrameTraffic({
  ...
  return (
    <g>
      ...
      >
        <rect x={pipeX - 18} y={y1} width={14} height={18} rx={2} fill="var(--color-accent)" />
        <text
          x={pipeX - 11}
          y={y1 + 12}
          textAnchor="middle"
      ...
      >
        <rect x={pipeX + 4} y={y2 - 18} width={14} height={18} rx={2} fill="var(--color-accent)" />
        <text
          x={pipeX + 11}
          y={y2 - 6}
          textAnchor="middle"
...
function NarrowAcceptSettle({
  ...
  return (
    <text key={accept} x={x} y={y} fontFamily="var(--font-mono)" fontSize={11}>
 ...
 * Only characters that change on a re-roll animate; keying on the value
 * itself forces a remount, so no manual diffing.
 */
export function AcceptSettle({ accept, y }: { accept: string; y: number }) {
  const SPLIT = Math.max(0, accept.length - 8);
  const head = accept.slice(0, SPLIT);
  const tail = accept.slice(SPLIT).split("");
  return (
    <text key={accept} x={0} y={y} fontFamily="var(--font-mono)" fontSize={11}>
      <tspan fill="var(--color-accent)">{head}</tspan>
      {tail.map((ch, i) => (
        <motion.tspan
          key={`${accept}-${i}`}
