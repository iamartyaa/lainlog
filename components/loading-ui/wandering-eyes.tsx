/**
 * WanderingEyes — vendored from the loading-ui registry
 * (https://loading-ui.com/r/wandering-eyes.json) so we can ship the
 * source without taking on a runtime dependency. The component is two
 * radial-gradient pupils inside translucent eye discs, animated by a
 * single `background-position` keyframe + a height-based blink keyframe.
 *
 * Adaptations vs upstream:
 *   - `cn` import path retargeted to `@/lib/utils/cn`.
 *   - Added a `prefers-reduced-motion: reduce` rule that pauses both
    ...
    <>
      <style>{`
        ...
        @media (prefers-reduced-motion: reduce) {
          [data-loading-ui-wandering-eyes-pupil] {
            animation: none !important;
            background-position: 0 0 !important;
            height: var(--loading-ui-wandering-eyes-eye) !important;
          }
        }
      `}</style>
      <span
