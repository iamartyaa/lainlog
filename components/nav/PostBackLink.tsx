 ...
 * post page. Default destination `/` ("back"); chapter pages override with
 * `href="/courses/<slug>"` and `label="course outline"`. Server component,
 * no client JS, no motion.
 *
 ...
 *
 * Aria: `aria-label="Back to <label>"` so screen-readers announce the
 * destination. The visible glyph + word stays "← <label>" — telegraphic
 * for sighted readers.
 *
...
export function PostBackLink({
  ...
  return (
    <Link
      href={href}
      aria-label={`Back to ${label === "back" ? "home" : label}`}
